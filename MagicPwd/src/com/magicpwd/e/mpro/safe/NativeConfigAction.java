...
public class NativeConfigAction extends AMproAction
{
    ...
    public void actionPerformed(java.awt.event.ActionEvent e)
    {
        ...
        if (!file.exists())
        {
            if (Lang.showFirm(mproPtn, MproRes.P30F7A51, "您选择的目录不存在，要创建此目录吗？") != javax.swing.JOptionPane.YES_OPTION)
            {
                return;
            }
        ...
        if (!file.isDirectory())
        {
            Lang.showMesg(mproPtn, MproRes.P30F7A1C, "请选择一个合适的目录！");
            return;
        }
        if (!file.canWrite())
        {
            Lang.showMesg(mproPtn, MproRes.P30F7A1D, "无法保存数据到您选择的目录，请确认您是否有足够的权限！");
            return;
        }
        ...
        {
            Lang.showMesg(mproPtn, MproRes.P30F7A1D, "备份目录不能为数据目录或其子目录！");
            return;
        }
