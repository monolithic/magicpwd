...
/**
 * Represents the collection of names (<em>symbols</em>) used for parsing an
 * expression (the context in which the parsing takes place).
 * <p>
 * 
 * Each symbol maps to either a {@link Function} or a constant.
 * <p>
 * 
 * A symbol is identified by the pair (name, arity). So a constant and a
 * function with the same name, or two function with the same name but with
 * different arity are distinct symbols.
 * <p>
 * 
 * Symbols functions as a stack of <em>frames</em>: when you pop the topmost
 * frame, all the symbols added in that frame dissapear (i.e. all the
 * modifications done between the push and the pop are undone).
 * <p>
 * 
 * Example:
 * 
 ...
 * 
 * <h3>Thread safety</h3>
 * The Symbols class is thread-safe (the same Symbols instance can be used by
 * multiple threads without locking).
 * <p>
 * 
 * The compile() methods are synchronized, so parallel compile() calls on the
 * same Symbols instance are serialized.
 */
...
public class Symbols {
	...
	private Hashtable<Symbol, Symbol> symbols = new Hashtable<Symbol, Symbol>();
	...
	public Symbols() {
		for (int i = 0; i < builtin.length; ++i) {
			add(builtin[i]);
			/*
			 * Symbol s = builtin[i]; symbols.put(s, s);
			 */
		}
		try {
	...
	public synchronized Complex evalComplex(String expression)
			throws SyntaxException {
		return compiler.compileSimple(this, expression).evalComplex();
	}

	/**
	 * Compiles an expression in the context of this Symbols. Does not modify
	 * the symbols.
	 * <p>
	 * 
	 ...
	 * 
	 * @param source
	 *            the expression; may contain '=' to denote a definition (with a
	 *            name).
	 * @return the function together with its eventual name.
	 *         <p>
	 * 
	 *         If this is not a definition (e.g. "1+1", "x^2"), the name is
	 *         null.
	 *         <p>
	 * 
	 *         If the expression is a constant (e.g. "1+1", "a=2"), the returned
	 *         Function is an instance of {@link Constant}.
	 * @throws SyntaxException
	 *             if there are errors compiling the expression.
	 */
	public synchronized FunctionAndName compileWithName(String source)
			throws SyntaxException {
	 ...
	 * When the frame is pop-ed the modifications that happened in it are
	 * reverted.
	 */
	 ...
	 *             if there were fewer <code>pushFrame</code> than
	 *             <code>popFrame</code>.
	 */
	...
	/**
	 * Returns all the symbols that were added in the top frame. (i.e. since the
	 * most recent pushFrame()).
	 */
	...
	private static final String defines[] = {
			...
			"asind(x)=asin(x) indeg", "acosd(x)=acos(x) indeg",
