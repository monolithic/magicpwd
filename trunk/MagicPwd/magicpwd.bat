...
SET TMP_CP="%MAGICPWD_HOME%\update\downloads\core\magicpwd-sql.jar"
...
set TMP_CP="%MAGICPWD_HOME%\magicpwd-sql.jar"
...
@rem "%LOCAL_JAVA%" -cp %TMP_CP% net.sourceforge.magicpwd_sql.client.Main %TMP_PARMS%

@rem To add translation working directories to your classpath edit and uncomment this line:
@rem start "MagicPwd SQL Client" /B "%LOCAL_JAVA%w" -Xmx256m -cp %TMP_CP%;<your working dir here> net.sourceforge.magicpwd_sql.client.Main %TMP_PARMS%
...
@rem start "MagicPwd SQL Client" /B "%LOCAL_JAVA%w" -Xmx256m -Dsun.java2d.noddraw=true -cp %TMP_CP%;<your working dir here> -Duser.language=<your language here> net.sourceforge.magicpwd_sql.client.Main %TMP_PARMS%

@rem Run with no command window. This may not work with older versions of Windows. Use the command above then.
start "MagicPwd SQL Client" /B "%LOCAL_JAVA%w" -Xmx256m -Dsun.java2d.noddraw=true -cp %MAGICPWD_CP% net.sourceforge.magicpwd_sql.client.Main %TMP_PARMS%
...
@rem "%LOCAL_JAVA%" -jar "%MAGICPWD_HOME%\magicpwd-sql.jar" %TMP_PARMS%
@rem start "MagicPwd SQL Client" /B "%LOCAL_JAVA%w" -Dsun.java2d.noddraw=true -jar "%MAGICPWD_HOME%\magicpwd-sql.jar" %TMP_PARMS%
